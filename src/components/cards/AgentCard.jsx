...
const AgentCard = ({ title, description, logo, agentUrl = "#" }) => {
  return (
    <div>
      <div className="group max-w-sm p-6 h-80 card">
        ...
        <div className="h-20 relative z-10 group/desc">
          {description?.length > 0 && (
            <>
              <p className="line-clamp-2 overflow-hidden font-normal text-black dark:text-white">
                {description}
              </p>
              <div className="absolute z-20 hidden group-hover/desc:block bg-[#616161] dark:bg-[#2a2d34f9] text-white text-xs rounded px-2 py-1 mt-1 left-1/2 -translate-x-1/2 w-max max-w-[250px] break-words whitespace-normal shadow-md">
        ...
        <hr className="mt-1 relative z-10" />
        <div className="flex mt-2 justify-between items-center text-[#3c5ba4] dark:text-[#e22a90] relative z-10">
          <div>Try Agent</div>
          <a href={agentUrl} target="_blank" rel="noopener noreferrer">
